...
class OMOPDataProcessor:
    ...
    def extract_patient_data(schema: Dict, person_id: int) -> np.array:
        ...
        MAINTAINS EXACT COMPATIBILITY with existing get_data function.
        """
        logger.info(f"Extracting data for person_id: {person_id}")
        
        # Validate schema
        validation = OMOPDataProcessor.validate_schema(schema)
        if not validation['valid']:
            logger.error(f"Schema validation failed: {validation['errors']}")
            return OMOPDataProcessor._generate_mock_data(schema, person_id)
                ...
                for feature_info in feature_definitions:
                    ...
                    if is_demographic or value_name in ["patient_sex", "patient_age"]:
                        if person_record_cache is None:
                            person_record_cache = DatabaseManager.get_person_by_id(person_id)
                        ...
                        if value_name == "patient_sex":
                            ...
                            if person_record_cache.year_of_birth:
                                try:
                                    ...
                                    raw_age = OMOPDataProcessor._calculate_age(birth_date)
                                    age_category = OMOPDataProcessor._map_age_to_uci_category(raw_age)
                    ...
                    if domain_key == "measurement":
                        measurements = DatabaseManager.get_patient_measurements(
                            person_id, 
                        ...
                        observations = DatabaseManager.get_patient_observations(
                            person_id,
                            [concept_id] if concept_id else None,
                            limit=1
                        )
                        ...
                        conditions = DatabaseManager.get_patient_conditions(
                            person_id,
                            [concept_id] if concept_id else None,
                            limit=1
                        )
...
def get_data(schema: Dict, person_id: Optional[int] = None) -> np.array:
    ...
    return OMOPDataProcessor.extract_patient_data(schema, person_id)
...
def load_custom_concepts_from_definitions():
    ...
    for value_name, definition in ALL_DEFINITIONS.items():
        ...
        existing_concept = DatabaseManager.get_concept_by_id(concept_id)
        if existing_concept:
    ...
    if concepts_to_add:
        DatabaseManager.bulk_insert_concepts(concepts_to_add)
